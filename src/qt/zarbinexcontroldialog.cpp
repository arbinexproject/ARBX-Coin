...
#include "ui_zarbinexcontroldialog.h"
...
std::list<std::string> ZArbinexControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZArbinexControlDialog::listMints;

ZArbinexControlDialog::ZArbinexControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZArbinexControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZArbinexControlDialog::~ZArbinexControlDialog()
{
    delete ui;
}

void ZArbinexControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZArbinexControlDialog::updateList()
{
...
void ZArbinexControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZArbinexControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZArbinex_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZArbinexControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZArbinexControlDialog::GetSelectedMints()
{
...
void ZArbinexControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
