...
#include "zarbinexcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zARBX ought to be enough for anybody." - Bill Gates, 2017
    ui->zARBXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzARBXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzARBX->setEnabled(false);
        ui->pushButtonMintzARBX->setToolTip(tr("zARBX is currently disabled due to maintenance."));

        ui->pushButtonSpendzARBX->setEnabled(false);
        ui->pushButtonSpendzARBX->setToolTip(tr("zARBX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zARBXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzARBX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zARBX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zARBX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zARBX in ") +
...
void PrivacyDialog::on_pushButtonSpendzARBX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zARBX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zARBX
        sendzARBX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zARBX
    sendzARBX();
}

void PrivacyDialog::on_pushButtonZArbinexControl_clicked()
{
    ZArbinexControlDialog* zARBXControl = new ZArbinexControlDialog(this);
    zARBXControl->setModel(walletModel);
    zARBXControl->exec();
}

void PrivacyDialog::setZArbinexControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzARBXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzARBX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Arbinex Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zARBXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zARBX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ARBINEX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zARBX</b>";
    ...
    // use mints from zARBX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZArbinexControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZArbinexControlDialog::GetSelectedMints();
    }
    ...
    // Clear zarbinex selector in case it was used
    ZArbinexControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zARBX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Arbinex, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zARBX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zARBX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zARBX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zARBX "));
    ui->labelzARBXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzARBXSyncStatus->setVisible(fShow);
}
