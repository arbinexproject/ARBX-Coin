...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ARBINEX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zARBX send
enum ZerocoinSpendStatus {
    ZARBINEX_SPEND_OKAY = 0,                            // No error
    ZARBINEX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZARBINEX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZARBINEX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZARBINEX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZARBINEX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZARBINEX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZARBINEX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZARBINEX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZARBINEX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZARBINEX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZARBINEX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZARBINEX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZARBINEX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZARBINEX_SPENT_USED_ZARBINEX = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZArbinexAutoBackups(bool fEnabled)
    {
