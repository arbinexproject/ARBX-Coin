...
*Setup instructions for a gitian build of Arbinex using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Arbinex
...
- [Building Arbinex](#building-arbinex)
...
Clone the git repositories for arbinex and gitian and then checkout the arbinex version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the arbinex repository
...
To build Arbinex (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the arbinex repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/arbinex/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Arbinex
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Arbinex.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit arbinex=${COMMIT} --url arbinex=${URL} ../Arbinex/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit arbinex=${COMMIT} --url arbinex=${URL} ../Arbinex/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit arbinex=${COMMIT} --url arbinex=${URL} ../Arbinex/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/arbinex-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/arbinex-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/arbinex-build.assert
```
