...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='arbinex.conf'
CONFIGFOLDER='/root/.arbinex'
COIN_DAEMON='arbinexd'
COIN_CLI='arbinex-cli'
...
function download_node() {
  ...
  wget -q $COIN_TGZ
  sudo chmod 775 arbinex-linux.zip
  apt install unzip 
  compile_error
#   tar xvzf $COIN_ZIP -C $COIN_PATH >/dev/null 2>&1
unzip arbinex-linux.zip 
cd arbinex-linux
sudo chmod 755 arbinexd
sudo chmod 755 arbinex-cli
cp * /usr/local/bin
chmod -R 775 *
cd ..
  cd - >/dev/null 2>&1
...
echo "arbinex MN installer Depends Starting"
